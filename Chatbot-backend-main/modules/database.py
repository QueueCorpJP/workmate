...
import json
import os
import hashlib  # 🚨 bcryptの代わりにhashlibを使用（AWS安定性のため）
...
# 🔒 パスワード暗号化関連ユーティリティ（bcrypt → hashlib SHA256に変更）
def hash_password(password: str) -> str:
    """パスワードをSHA256でハッシュ化（AWS安定性のため）"""
    # 🚨 bcryptをSHA256に置き換え（AWS互換性向上）
    salt = str(uuid.uuid4())[:8]  # 簡易的なsalt
    salted_password = f"{password}{salt}"
    hashed = hashlib.sha256(salted_password.encode('utf-8')).hexdigest()
    return f"{hashed}${salt}"  # ハッシュとsaltを結合
