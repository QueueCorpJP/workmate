...
class EmbeddingDiagnostics:
    ...
    async def check_gemini_rate_limits(self):
        """原因③ Gemini API制限チェック"""
    ...
    def check_async_processing(self):
        """原因① 非同期処理の確認"""
        logger.info("🔍 原因① 非同期embedding処理チェック")
        
        try:
            # AUTO_GENERATE_EMBEDDINGS設定を確認
            auto_embed = os.getenv("AUTO_GENERATE_EMBEDDINGS", "false").lower()
            logger.info(f"📋 AUTO_GENERATE_EMBEDDINGS設定: {auto_embed}")
            
            if auto_embed == "true":
                logger.info("✅ 自動embedding生成が有効")
                
                # document_processor.pyの処理フローを確認
                # 実際のコードを読み込んで非同期処理の実装を確認
                try:
                    with open("modules/document_processor.py", "r", encoding="utf-8") as f:
                        content = f.read()
                        
                    if "_generate_embeddings_batch" in content:
                        logger.info("✅ embedding生成メソッドが存在")
                        
                        if "await" in content and "asyncio" in content:
                            logger.info("✅ 非同期処理が実装されている")
                        else:
                            logger.warning("⚠️ 非同期処理の実装が不完全の可能性")
                    else:
                        logger.error("❌ embedding生成メソッドが見つからない")
                        
                except Exception as file_error:
                    logger.error(f"❌ ファイル読み込みエラー: {file_error}")
            else:
                logger.warning("⚠️ 自動embedding生成が無効")
                
            return auto_embed == "true"
            
        except Exception as e:
            logger.error(f"❌ 非同期処理チェックエラー: {e}")
            return False
    ...
    async def run_full_diagnosis(self):
        ...
        # 原因① 非同期処理
        async_processing_ok = self.check_async_processing()
        diagnosis_results['async_processing'] = async_processing_ok
        ...
        if not diagnosis_results['async_processing']:
            issues_found.append("① embedding処理の非同期化が未完了")
        ...
        if not diagnosis_results['async_processing']:
            logger.info("🔧 非同期処理改善:")
            logger.info("   - AUTO_GENERATE_EMBEDDINGS=true に設定")
            logger.info("   - document_processor.pyの非同期処理を確認")
...
async def main():
    ...
    with open(report_file, 'w', encoding='utf-8') as f:
        ...
        f.write("=" * 60 + "\n")
        f.write(f"非同期処理: {'✅' if results['async_processing'] else '❌'}\n")
